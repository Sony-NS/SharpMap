...
namespace DelftTools.Utils.Collections.Extensions
{
    ...
    public static class ListExtensions
    {
        ...
        public static int BinaryHintedSearch(this IList semiSortedList, object value)
        {
            ...
            endIndex = endIndex < realValues.Count - 1 ? endIndex : realValues.Count - 1;
            ...
            //is below code required? theoretically it can be reached.
            for (int j = 0; j < realValues.Count; j++)
            {
                if (realValues[j].Equals(value))
                {
                    return j;
