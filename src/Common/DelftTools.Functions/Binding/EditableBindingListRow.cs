...
namespace DelftTools.Functions.Binding
{
    public abstract class EditableBindingListRow : IEditableObject
    {
        ...
        protected void BeginEditManually()
        {
            if (!inEditMode)
            {
                FillTransientValues();
        ...
        protected void SetColumnValueTransient(int columnIndex, object value)
        {
            transientValues[columnIndex] = value;
            transientValuesChanged[columnIndex] = true;
        }

        protected object GetColumnValueTransient(int columnIndex)
        {
        ...
        private void CommitTransientValues()
        {
            if (inEditMode)
            {
                throw new NotSupportedException("Can only commit when no longer in edit mode"); //make sure inEditMode is set to false first!
            }
