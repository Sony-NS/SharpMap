...
{
    [Serializable]
    public class MultiDimensionalArray<T> : MultiDimensionalArray, IMultiDimensionalArray<T>, IMultiDimensionalArray
    {
        ...
        public virtual void AddRange(IEnumerable<T> enumerable)
        {
            if (Rank != 1)
            {
                throw new InvalidOperationException("Add range is only supported for arrays of Rank1. Use InsertAt instead");
            }
            //this is bad (slow) we have nice doubles and box them into objects...performance drama..
            var values = enumerable.Cast<object>().ToArray();
            InsertAt(0,Count,values.Length,values);
        }
        ...
        private bool? isReferenceTyped;
        protected override bool IsReferenceTyped
        {
            get
            {
                //cache this value for speed
                if (isReferenceTyped == null)
                {
                    isReferenceTyped = !typeof (T).IsValueType;
                }
                return (bool) isReferenceTyped;
            }
        }
...
*/
        ...
        {
            return MultiDimensionalArrayHelper.GetInsertionIndex((T)valuesToInsert[0], values2);
        }
