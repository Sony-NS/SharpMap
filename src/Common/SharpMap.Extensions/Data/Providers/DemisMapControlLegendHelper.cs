...
namespace SharpMap.Extensions.Data.Providers
{
    public class DemisMapControlLegendHelper
    {
        ...
        private static Ranges<T> ParseRanges<T>(Legend legend)
        {
            ...
            int oldIntMaxValue = (int) Double.Parse(legend.dict["MinValue"].Replace('.', ','), CultureInfo.InvariantCulture);
            double oldDoubleMaxValue = Double.Parse(legend.dict["MinValue"].Replace('.', ','), CultureInfo.InvariantCulture);
