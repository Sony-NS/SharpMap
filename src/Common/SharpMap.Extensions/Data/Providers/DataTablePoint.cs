...
namespace SharpMap.Data.Providers
{
    ...
    public class DataTablePoint : Unique<long>, IFeatureProvider, IDisposable
    {
        ...
        public ICollection<IGeometry> GetGeometriesInView(IEnvelope bbox, double minGeometrySize)
        {
            ...
            DataRow[] drow = Table.Select(strSQL);

            foreach (DataRow dr in drow)
            {
                features.Add(SharpMap.Converters.Geometries.GeometryFactory.CreatePoint((double) dr[XColumn],
                                                                                        (double) dr[YColumn]));
            }
        ...
        public ICollection<int> GetObjectIDsInView(IEnvelope envelope)
        {
            ...
            string strSQL = XColumn + " > " + envelope.MinX.ToString(Map.numberFormat_EnUS) + " AND " +
                            XColumn + " < " + envelope.MaxX.ToString(Map.numberFormat_EnUS) + " AND " +
                            YColumn + " > " + envelope.MinY.ToString(Map.numberFormat_EnUS) + " AND " +
                            YColumn + " < " + envelope.MaxY.ToString(Map.numberFormat_EnUS);
        ...
        {
            return GetFeatures(geom.EnvelopeInternal);
        }
        ...
        public IList GetFeatures(IEnvelope bbox)
        {
            DataRow[] rows;
