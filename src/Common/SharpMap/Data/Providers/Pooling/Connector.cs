...
namespace SharpMap.Data.Providers.Pooling
{
	...
	internal class Connector
	{
		...
		private IFileBased pooledEntity;
		/// <summary>
		/// Used to connect to the data source. 
		/// </summary>
		internal IFileBased PooledEntity
		{
			get { return pooledEntity; }
			set
			{
				if (this.InUse)
				{
					throw new ApplicationException("Entity cannot be modified if connection is open.");
				}
				pooledEntity = value;
			}
		}
		...
		internal void Open()
		{
		    PooledEntity.Open(PooledEntity.Path);
		}
