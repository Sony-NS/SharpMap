...
namespace SharpMap.Data.Providers
{
    ...
    public class MsSqlSpatial : Unique<long>, IFeatureProvider, IFileBased
	{
		...
		public ICollection<int> GetObjectIDsInView(IEnvelope envelope)
		{
			...
			using (SqlConnection conn = new SqlConnection(this.ConnectionString))
			{
				string strSQL = "SELECT * FROM ST.FilterQuery('" + this.Table + "', '" + this.GeometryColumn + "', " + this.BuildEnvelope(envelope) + ")";
		...
		public GeoAPI.Geometries.IEnvelope GetExtents()
		{
			using (SqlConnection conn = new SqlConnection(_ConnectionString))
			{
                ...
                //string strSQL = String.Format("SELECT g.{0}.STAsBinary() FROM {1} g ", this.GeometryColumn, this.Table);
                //if (!String.IsNullOrEmpty(this.DefinitionQuery))
                //    strSQL += " WHERE " + this.DefinitionQuery;
