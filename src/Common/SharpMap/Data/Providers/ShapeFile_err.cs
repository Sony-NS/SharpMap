...
namespace SharpMap.Data.Providers
{
    ...
    public class ShapeFile : SharpMap.Data.Providers.IUpdateableProvider, IDisposable
    {
        ...
        private SharpMap.CoordinateSystems.CoordinateSystem _CoordinateSystem;
        ...
        public ICollection<IGeometry> GetGeometriesInView(IEnvelope bbox)
        {
            checkOpen();
            EnableReading();
            ...
            Collection<IGeometry> geometries = new Collection<IGeometry>(objectlist.Count);
        ...
        /// <returns></returns>
        /// <exception cref="InvalidShapefileOperationException">Thrown if method is called and the shapefile is closed. Check <see cref="IsOpen"/> before calling.</exception>
        public List<uint> GetObjectIDsInView(IEnvelope bbox)
        {
            checkOpen();
            EnableReading();
        ...
        /// <returns></returns>
        /// <exception cref="InvalidShapefileOperationException">Thrown if method is called and the shapefile is closed. Check <see cref="IsOpen"/> before calling.</exception>
        public virtual IFeature GetFeature(uint RowID)
        {
            checkOpen();
			...
			if (System.IO.File.Exists(filename + ".sidx"))
			{
				try
                {
                    using (FileStream indexStream = new FileStream(filename + ".sidx", FileMode.Open, FileAccess.Read, FileShare.Read))
					    return QuadTree.FromFile(indexStream);
				}
				catch(SharpMap.Utilities.SpatialIndexing.QuadTree.ObsoleteFileFormatException)
				{
        ...
        private SharpMap.Converters.Geometries.GeometryFactory ReadMultiPoint()
        {
        ...
        private GeometryFactory ReadPolyLine()
        {
            int parts;
            int points;
            int[] segments;
        ...
        private SharpMap.Converters.Geometries.GeometryFactory ReadPolygon()
        {
            int parts;
            int points;
            int[] segments;
        ...
        private void WritePolySegments(IEnvelope bbox, int[] parts, SharpMap.Geometries.Point[] points)
        {
