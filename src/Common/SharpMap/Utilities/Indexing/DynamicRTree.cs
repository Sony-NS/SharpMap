...
namespace SharpMap.Utilities.Indexing
{
	...
	public class DynamicRTree : RTree, System.Runtime.Serialization.ISerializable
	{
        ...
        private static Node ReadNode(DynamicRTree tree, BinaryReader br)
        {
            ...
            node.Box = new GisSharpBlog.NetTopologySuite.Geometries.Envelope(br.ReadDouble(), br.ReadDouble(), br.ReadDouble(), br.ReadDouble());
            ...
            if (isLeaf)
            {
                ...
                for (int i = 0; i < featureCount; i++)
                {
                    ...
                    entry.Box = new GisSharpBlog.NetTopologySuite.Geometries.Envelope(br.ReadDouble(), br.ReadDouble(), br.ReadDouble(), br.ReadDouble());
        ...
        private void SaveNode(Node node, BinaryWriter writer)
        {
            ...
            writer.Write(node.Box.MinX);
            writer.Write(node.Box.MinY);
            writer.Write(node.Box.MaxX);
            writer.Write(node.Box.MaxY);
            ...
            if (node is LeafNode)
            {
                ...
                {
                    writer.Write(entry.Box.MinX);
                    writer.Write(entry.Box.MinY);
                    writer.Write(entry.Box.MaxX);
                    writer.Write(entry.Box.MaxY);
