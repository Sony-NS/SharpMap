...
namespace SharpMap.UI.Tools
{
    ...
    public class LegendTool : LayoutComponentTool
    {
        ...
        private DateTime? GetLayersTime()
        {
            ...
            //statement is very strange but is the same behaviour as before..should depend on ITimeNavigatable here..
            var timeDependendCoverageGroupLayer =
                Map.GetAllLayers(true).OfType<ICoverageLayer>().FirstOrDefault(l => l is IGroupLayer && l.Visible && l.ShowInLegend);
            if (timeDependendCoverageGroupLayer != null)
            {
                return GetCurrentTimeFromCoverageLayer(timeDependendCoverageGroupLayer);
            }
            return currentTime;
        }
        ...
        private static DateTime? GetCurrentTimeFromCoverageLayer(ICoverageLayer coverageLayer)//, DateTime? currentTime)
        {
            ...
            if (coverageLayer.Coverage.Time != null)
            {
                ...
                {
                    if (coverageLayer is INetworkCoverageGroupLayer)
                    {
                        var networkCoverageGroupLayer = (INetworkCoverageGroupLayer)coverageLayer;
                        currentTime = ((NetworkCoverageFeatureCollection) networkCoverageGroupLayer.SegmentLayer.DataSource).TimeSelectionStart;
                    }
                    else if (coverageLayer.Coverage.Time.Values.Count > 0)
                    {
