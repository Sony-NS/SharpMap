...
namespace DelftTools.Tests.Core
{
    ...
    public class ProjectTest
    {
        ...
        [Test]
        public void DataItemShouldBeRemovableFromProject()
        {
            Project project = new Project();
            var item = new DataItem("item");
            project.RootFolder.Add(item);
            var itemInProject = project.RootFolder.DataItems.Where(i => i.Name == "item").First();
            Assert.IsTrue(itemInProject.IsRemoveable);
        }
        ...
        [Test]
        public void GetUniqueProjectItemName()
        {
            var project = new Project();
            var folder = new Folder();
            project.RootFolder.Add(folder);

            Assert.AreEqual("Folder1", project.GetUniqueProjectItemName<Folder>());

            //add another one
            project.RootFolder.Add(new Folder { Name = "Folder1" });

            //next name should be folder2
            Assert.AreEqual("Folder2", project.GetUniqueProjectItemName<Folder>());
        }

        [Test]
        public void GetUniqueProjectItemNameForItemInDataItem()
        {
            var project = new Project();
            var url = new Url {Name = "Url1"};
            var dataItem = new DataItem(url);
            project.RootFolder.Add(dataItem);

            //assert the next added url should be name Url2
            Assert.AreEqual("Url2", project.GetUniqueProjectItemName<Url>());
        }
